...
class AMHelper
{
	...
	*/
	static AM getAddressingMode(String op)
	{
		if (op.charAt(0)=='[')
		{
			if (op.charAt(op.length()-1)==']')
			{
				...
				{
					// If no. of '+' = 1, and op contains SI or DI, then register relative addressing mode (eg, [BX+SI]).
					String o[] = op.split("\\+");
					if (o[1].equals("SI") || o[1].equals("DI"))
						return AM.REGREL;									//REGISTER RELATIVE
					
					// If no. of '+' = 1, and op does not contain SI or DI, then based indexed addressing mode (eg, [BX+7]).
					return AM.BASIND;										//BASED INDEXD
				}
				else if (plusCount==2)
				{
					// If no. of '+' = 2, then relative based indexed addressing mode (eg, [BX+SI+7]).
		...
		if (op.charAt(0)=='#')
			// If op starts with '#', then immediate addressing mode (eg, #6DH).
		{
			...
			// Else direct addressing mode (eg, DATA).
	...
	/* addressingModeIntermediate: 
									OBJECTIVE 	- An intermediate function to return the operand type based on addressing mode.
